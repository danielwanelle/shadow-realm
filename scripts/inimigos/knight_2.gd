...
func drop_itens():
	var pocao_defesa = preload("res://scenes/itens/pocao_defesa.tscn").instantiate()
	get_parent().add_child(pocao_defesa)
	pocao_defesa.position = position + Vector2(randf_range(-20, 20), 0)
